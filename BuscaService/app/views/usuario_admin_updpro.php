<?php 
    ...
    if($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        $query = "UPDATE `busca_service`.`cliente` SET `nome` = :nome, `telefone` = :telefone, `cep` = :cep, `estado` = :estado, `cidade` = :cidade, `bairro` = :bairro, `status` = :status 
                    WHERE idcli = :idcli";
                ...
                <form action="" method="post" class="box">
                    <fieldset>
                        ...
                        <div class="dadosPessoais">
                            <div class="inputBox">
                                <input type="text" name="nome" id="nome" class="inputUser" required autofocus
                                    value="<?=isset($row)? $row['nome'] : ''?>">
                                <label for="nome" class="labelInput">Nome</label>
                            </div>

                            <div class="inputBox">
                                <input type="email" name="email" id="email" class="inputUser"  readonly autofocus
                                    value="<?=isset($row)? $row['email'] : ''?>">
                                    <label for="email" class="labelInput <?=isset($row) && !empty($row['email']) ? 'active' : ''?>">E-mail:</label>
                            </div>

                            <div class="inputBox">
                                <input type="text" name="cpf" id="cpf" class="inputUser"  readonly autofocus
                                    value="<?=isset($row)? $row['cpf'] : ''?>">
                            ...
                            <div class="inputBox">
                                <input type="tel" name="telefone-whatsapp" id="telefone-whatsapp" class="inputUser"
                                    minlength="14" maxlength="14" required autofocus
                                    value="<?=isset($row)? $row['telefone'] : ''?>">
                        ...
                        <div class="endereco">
                            <div class="inputBox">
                                <input type="text" id="cep" name="cep" class="inputUser" maxlength="8" minlength="8"
                                    required autofocus value="<?=isset($row)? $row['cep'] : ''?>">
                                <label for="cep" class="labelInput">CEP:</label><br>
                            </div>

                            <div class="inputBox">
                                <input type="text" name="estado" id="estado" class="inputUser" required autofocus
                                    value="<?=isset($row)? $row['estado'] : ''?>">
                                <label for="uf" class="labelInput">Estado:</label>
                            </div>

                            <div class="inputBox">
                                <input type="text" name="cidade" id="cidade" class="inputUser" required autofocus
                                    value="<?=isset($row)? $row['cidade'] : ''?>">
                                <label for="cidade" class="labelInput">Cidade:</label>
                            </div>

                            <div class="inputBox">
                                <input type="text" name="bairro" id="bairro" class="inputUser" required autofocus
                                    value="<?=isset($row)? $row['bairro'] : ''?>">
