<?php 
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        $query = "INSERT INTO `busca_service1`.`profissional` (`nome`, `titulo`, `email`, `senha`, `cpf`, `telefone`, `telefone2`, `cep`, `estado`, `cidade`, `bairro`)
                    VALUES (:nome, :titulo, :email, :senha, :cpf, :telefone, :telefone2, :cep, :estado, :cidade, :bairro)";
        ...
        if ($stmt->rowCount()) {
            ...
            if (isset($_POST['servico']) && is_array($_POST['servico'])) {
                ...
                $query = "INSERT INTO `busca_service1`.`profissional_has_servico` (`idpro`, `idserv`) VALUES (:idpro, :idserv)";
                $stmt = $dbh->prepare($query);
            ...
            header('location: testepro.php?error=Erro ao inserir o profissional!');
        }
    }
    ...
    $query = "SELECT * FROM `busca_service1`.`servico` ORDER BY categoria, nome";
    $stmt = $dbh->prepare($query);
                ...
                <form action="" method="post" class="box" enctype="multipart/form-data">
                    <fieldset class="main_form">
                        ...
                        <div class="dadosPessoais">
                            ...
                            <div class="inputBox">
                                <input type="tel" name="telefone" id="telefone-whatsapp" class="inputUser"
                                    minlength="14" maxlength="14" required>
                                <label for="telefone-whatsapp" class="labelInput">Celular (WhatsApp):</label>
                            </div>

                            <div class="inputBox">
                                <input type="tel" name="telefone2" id="telefone-geral" class="inputUser"
                                    minlength="14" maxlength="14" required>
